...
// ==========================================
// Tarea B: Filtro de Convolución 2D (Vectorizado con SPMD/SIMD)
// ==========================================
void applyGaussianFilter(const std::vector<Pixel>& input, std::vector<Pixel>& output) {
    ...
    // SPMD a nivel de Hilos (Nodos de procesamiento lógico)
    ...
    for (int y = 0; y < HEIGHT; ++y) {
        for (int x = 0; x < WIDTH; ++x) {
            ...
            for (int ky = -RADIUS; ky <= RADIUS; ++ky) {
                // Elevamos este cálculo fuera del bucle SIMD para evitar dependencias
                int ny = std::max(0, std::min(HEIGHT - 1, y + ky));
                
                // SPMD a nivel de Datos (Registros Vectoriales AVX/SSE)
                // Usamos reduction para evitar colisiones en la suma paralela
...
// ==========================================
// Tarea C: Histograma con Exclusión Mutua (Atomic)
// ¡ADVERTENCIA: Diseñado para demostrar cuello de botella!
// ==========================================
std::vector<int> calculateColorHistogramAtomic(const std::vector<Pixel>& image) {
    const int NUM_COLORS = 256 * 256 * 256; 
    std::vector<int> global_histogram(NUM_COLORS, 0);

    // Un solo bucle paralelo donde todos los hilos atacan el mismo arreglo
    #pragma omp parallel for schedule(static)
    for (size_t i = 0; i < image.size(); ++i) {
        const Pixel& p = image[i];
        int color_idx = (p.r << 16) | (p.g << 8) | p.b;

        // Exclusión mutua: Obliga a los hilos a formarse para modificar esta dirección de memoria
        #pragma omp atomic
        global_histogram[color_idx]++;
    }
...
int main() {
    ...
    std::vector<int> final_histogram = calculateColorHistogramAtomic(blurred_image);
